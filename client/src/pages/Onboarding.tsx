...
const Onboarding = () => {
  ...
  const handleGoalSelect = (goalValue: Goal) => {
    const age = Number(formData.age);
    const range =
      ...
      <div className="px-6 mb-8 onboarding-wrapper">
        <div className="flex gap-2 max-w-2xl">
          {[1, 2, 3].map((s) => (
            <div
              key={s}
              className={`h-1.5 flex-1 rounded-full transition-all duration-300 ${s <= step ? "bg-emerald-500" : "bg-slate-200 dark:bg-gray-800"}`}
            />
          ))}
        </div>
      ...
      <div className="flex-1 px-6 onboarding-wrapper">
        ...
        {step === 3 && (
          <div className="space-y-6 onboarding-wrapper">
            ...
            <div className="border-t bg-slate-200 dark:border-slate-700 my-6 max-w-lg"></div>

            {/* daily targets */}
            <div className="space-y-8 max-w-lg">
              <h3 className="text-base font-medium text-slate-800 dark:text-white mb-4">
                Daily Targets
              </h3>
